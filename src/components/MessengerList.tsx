  ...
  const [contacts] = useState<Contact[]>([
    {
      ...
      avatar: '👤',
      lastMessage: 'Сложно сказать, почему активно развивающиеся страны третьего мир...',
      time: '10:43',
      isOnline: true,
      hasUnread: true,
      isRead: false
    },
    {
      ...
      avatar: '👤',
      lastMessage: 'Надо отправить отчёт за прошедший квартал, будет так же АВР по процессам...',
      time: '10:43',
      isOnline: false,
      hasUnread: false,
      isRead: true
    },
    {
      ...
      avatar: '👤',
      lastMessage: 'Сложно сказать, почему активно развивающиеся страны третьего мира...',
      time: '10:43',
      isOnline: true,
      hasUnread: false,
      isRead: false
          ...
          {contacts.map((contact) => (
            ...
            >
              <div className="messenger__contact-avatar">
                {contact.avatar}
                {contact.isOnline && <div className="messenger__online-indicator"></div>}
              </div>
              <div className="messenger__contact-info">
                <div className="messenger__contact-header">
                  <span className="messenger__contact-name">{contact.name}</span>
                  <span className="messenger__contact-time">{contact.time}</span>
                </div>
                <div className="messenger__contact-message">
                  {contact.isRead && <span className="messenger__read-icon">✓</span>}
                  <span className="messenger__message-text">{contact.lastMessage}</span>
                </div>
              </div>
              {contact.hasUnread && <div className="messenger__unread-indicator"></div>}
            </div>
          ))}
        </div>
      </div>
          ...
          <>
            <div className="messenger__chat-header">
              <div className="messenger__chat-user">
                <div className="messenger__chat-avatar">
                  ...
                  {selectedContact.isOnline && <div className="messenger__online-indicator"></div>}
                </div>
                <span className="messenger__chat-name">{selectedContact.name}</span>
              </div>
            ...
            <div className="messenger__messages">
              {messages.map((message) => (
                <div key={message.id} className={`messenger__message ${message.isOwn ? 'messenger__message--own' : ''}`}>
                  <div className="messenger__message-content">
                    {message.text}
                  </div>
                  <div className="messenger__message-footer">
                    <span className="messenger__message-time">{message.time}</span>
                    {message.isOwn && message.isRead && <span className="messenger__message-read">✓</span>}
                  </div>
                </div>
              ))}
